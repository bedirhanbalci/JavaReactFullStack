...
public class Main {
    public static void main(String[] args) {
        ...
        users.add(new User(2, "Engin", "Demiroğ", 30));
        users.add(new User(3, "İrem", "Balcı", 16));

        List<User> users2 = users.stream().filter((user) -> user.getAge() > 18) // one line return keywordu gerekmez
                .toList();
        users2.forEach((user) -> System.out.println(user.getName()));

        // "findFirst" ilgili filtre ile bir kayıta rastlamayabilir çünkü türü optional
        User user = users
                .stream()
...
//                .sorted((user1, user2) -> user1.getName().compareTo(user2.getName()))
        ...
        // Sıralama yaptığımızda türkçe karakterleri yanlış karşılaştırdığı için bunu ekledik
        Collator collator = Collator.getInstance(new Locale("tr", "TR"));

        List<User> sortedUsers2 = users
                .stream()
                // Classİsmi::fonksiyonİsmi => fonksiyon referansı transferi. sorted'ın kısa kullanımı
