...
public interface CorporateRepository extends JpaRepository<Corporate, Integer> {
    ...
    @Query("Select new com.tobeto.rent.a.car.demo.services.dtos.corporate.responses.GetListCorporateResponse (c.id, c.companyName, c.contactName, c.contactTitle, c.customer)" +
            "From Corporate c WHERE c.contactTitle ILIKE '%Manager%'")
    List<GetListCorporateResponse> findByContactTitleILike();

    @Query("Select new com.tobeto.rent.a.car.demo.services.dtos.corporate.responses.GetListCorporateResponse (c.id, c.companyName, c.contactName, c.contactTitle, c.customer)" +
            "From Corporate c WHERE c.companyName = 'Tobeto' AND c.contactTitle = 'Sales Manager'")
    List<GetListCorporateResponse> findByCompanyNameAndContactTitle();

    @Query("Select new com.tobeto.rent.a.car.demo.services.dtos.corporate.responses.GetListCorporateResponse (c.id, c.companyName, c.contactName, c.contactTitle, c.customer)" +
            "From Corporate c ORDER BY c.contactName ASC")
