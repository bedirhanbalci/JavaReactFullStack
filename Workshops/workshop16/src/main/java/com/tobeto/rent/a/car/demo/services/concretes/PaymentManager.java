...
//        return paymentRepository.findByMonthlyPrice(monthlyPrice);
        ...
        return paymentRepository.findByMonthlyPrice(monthlyPrice).stream().map((payment) -> {
            return new GetListPaymentResponse(payment.getId(), payment.getDailyPrice(), payment.getWeeklyPrice(), payment.getMonthlyPrice(), payment.getCustomer());
        }).toList();
    }
}