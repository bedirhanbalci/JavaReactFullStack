...
public class CorporateManager implements CorporateService {
    ...
    public void add(AddCorporateRequest addCorporateRequest) {
        ...
        if (corporateRepository.existsCorporateByTaxNumber(addCorporateRequest.getTaxNumber())) {
            throw new RuntimeException("Aynı vergi numarasını bir daha giremezsiniz!");
        }
...
//        return corporateRepository.findByContactTitleILike();
        ...
        return corporateRepository.findByContactTitleILike().stream().map((corporate) -> {
            return new GetListCorporateResponse(corporate.getId(), corporate.getCompanyName(), corporate.getContactName(), corporate.getContactTitle(), corporate.getCustomer());
        }).toList();
    }
...
//        return corporateRepository.findByCompanyNameAndContactTitle();
        ...
        return corporateRepository.findByCompanyNameAndContactTitle().stream().map((corporate) -> {
            return new GetListCorporateResponse(corporate.getId(), corporate.getCompanyName(), corporate.getContactName(), corporate.getContactTitle(), corporate.getCustomer());
        }).toList();
    }
...
//        return corporateRepository.findOrderByContactName();
        ...
        return corporateRepository.findOrderByContactName().stream().map((corporate) -> {
            return new GetListCorporateResponse(corporate.getId(), corporate.getCompanyName(), corporate.getContactName(), corporate.getContactTitle(), corporate.getCustomer());
        }).toList();
    }
}