...
public class DateManager implements DateService {
    ...
    public void add(AddDateRequest addDateRequest) {
        ...
        if (addDateRequest.getStartDate().isAfter(LocalDate.now())) {
            throw new RuntimeException("Başlangıç tarihi bugünden küçük veya bugüne eşit olmalıdır!");
        }

        if (addDateRequest.getStartDate().isBefore(addDateRequest.getStartDate())) {
            throw new RuntimeException("Bitiş tarihi, başlangıç tarihinden büyük olmalıdır!");
        }

        if (addDateRequest.getExtendDate() != null && (addDateRequest.getExtendDate().isBefore(addDateRequest.getStartDate()) || addDateRequest.getExtendDate().isBefore(addDateRequest.getEndDate()))) {
            throw new RuntimeException("Uzatma tarihi, başlangıç tarihinden ve bitiş tarihinden büyük olmalıdır!");
        }
    ...
    public void update(UpdateDateRequest updateDateRequest) {
        ...
        if (updateDateRequest.getStartDate().isAfter(LocalDate.now())) {
            throw new RuntimeException("Başlangıç tarihi bugünden küçük veya bugüne eşit olmalıdır!");
        }

        if (updateDateRequest.getStartDate().isBefore(updateDateRequest.getStartDate())) {
            throw new RuntimeException("Bitiş tarihi, başlangıç tarihinden büyük olmalıdır.");
        }

        if (updateDateRequest.getExtendDate() != null && (updateDateRequest.getExtendDate().isBefore(updateDateRequest.getStartDate()) || updateDateRequest.getExtendDate().isBefore(updateDateRequest.getEndDate()))) {
            throw new RuntimeException("Uzatma tarihi, başlangıç tarihinden ve bitiş tarihinden büyük olmalıdır.");
        }
