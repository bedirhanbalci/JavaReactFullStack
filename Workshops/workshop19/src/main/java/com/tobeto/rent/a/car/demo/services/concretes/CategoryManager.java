...
public class CategoryManager implements CategoryService {
    ...
    public void add(AddCategoryRequest addCategoryRequest) {
        ...
        if (categoryRepository.existsCategoryByName(addCategoryRequest.getName())) {
            throw new RuntimeException("Birden fazla aynı kategori olamaz!");
        }

        if (addCategoryRequest.getName().length() > 3) {
            throw new RuntimeException("Kategori 3 haneden büyük olmalıdır!");
        }
    ...
    public void update(UpdateCategoryRequest updateCategoryRequest) {
        ...
        if (categoryRepository.existsCategoryByName(updateCategoryRequest.getName())) {
            throw new RuntimeException("Birden fazla aynı kategori olamaz! Lütfen koşullara uygun giriş yapınız!");
        }

        if (updateCategoryRequest.getName().length() > 3) {
            throw new RuntimeException("Kategori 3 haneden büyük olmalıdır! Lütfen koşullara uygun giriş yapınız!");
        }
